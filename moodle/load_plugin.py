...
root@ip-192-168-27-221:/home/ubuntu/moodle-docker# ls
Dockerfile  README.md  entrypoint-cron.sh  entrypoint.sh  load_plugins.py  moodle  moodle-config.php  moodle-nginx.conf  notes  plugins
root@ip-192-168-27-221:/home/ubuntu/moodle-docker# vim load_plugins.py 
root@ip-192-168-27-221:/home/ubuntu/moodle-docker# cat load_plugins.py 
import sys
import os
import requests
import json
...
class PluginDB:
    ...
    def find_most_recent_url_for_version(self, plugin, moodle_version):
        ...
        # If no versions match, get the most recent release
        return sorted(self.db[plugin]['versions'], key=lambda x: x['version'], reverse=True)[0]['downloadurl']
